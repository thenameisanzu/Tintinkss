...
function CollectionRow({
  ...
  return (
    <motion.div
      ...
      className={`group border-t border-porcelain/15 py-8 md:py-12 px-4 sm:px-6 -mx-4 sm:-mx-6 rounded-2xl flex flex-col md:flex-row gap-4 md:gap-10 relative cursor-pointer transition-colors duration-400 hover:bg-porcelain/[0.04] ${
        index % 2 === 1 ? "md:flex-row-reverse md:text-right" : ""
      }`}
    >
      <div className={`md:w-2/5 flex items-center gap-3.5 ${index % 2 === 1 ? "md:justify-end" : ""}`}>
        <h3 className="font-display font-medium text-4xl md:text-6xl text-porcelain group-hover:text-rust-light transition-all duration-300 group-hover:translate-x-1.5">
          {item.title}
        </h3>
        <ArrowUpRight
          size={26}
      ...
      <div className={`md:w-3/5 flex flex-col gap-3.5 ${index % 2 === 1 ? "md:items-end" : ""}`}>
        <p className="font-display italic font-normal text-lg md:text-xl text-porcelain/75 group-hover:text-porcelain/90 max-w-md leading-relaxed transition-colors duration-300">
          {item.description}
        </p>
        <div className="flex items-center gap-3 pt-1">
          ...
          <span className="text-xs font-medium tracking-wide text-porcelain/50 group-hover:text-rust transition-colors underline-offset-4 group-hover:underline">
            enquire piece &rarr;
          </span>
        </div>
      </div>
